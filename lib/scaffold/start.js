...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for recryptd is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and recryptcore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed recryptcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './recryptcore-node.json');
  ...
  node.on('ready', function() {
    log.info('Recryptcore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in recryptcore-node services directory
    ...
    if (servicePackage.recryptcoreNode) {
      serviceModule = service.name + '/' + servicePackage.recryptcoreNode;
    }
