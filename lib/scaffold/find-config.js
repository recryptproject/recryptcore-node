...
/**
 * Will return the path and recryptcore-node configuration
...
function findConfig(cwd) {
  ...
  while (!fs.existsSync(path.resolve(directory, 'recryptcore-node.json'))) {
  ...
  return {
    path: directory,
    config: require(path.resolve(directory, 'recryptcore-node.json'))
  };
}
