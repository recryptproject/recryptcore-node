...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'recryptcore-lib': 'https://github.com/recryptproject/recryptcore-lib.git#master',
    'recryptcore-node': 'https://github.com/recryptproject/recryptcore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/recryptcore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the recryptcore-node directory and configuration
