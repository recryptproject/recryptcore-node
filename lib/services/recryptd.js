...
var RecryptRPC = require('recryptd-rpc');
...
/**
 * Provides a friendly event driven API to recryptd in Node.js. Manages starting and
 * stopping recryptd as a child process for application support, as well as connecting
 * to multiple recryptd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Recrypt(options) {
  if (!(this instanceof Recrypt)) {
    return new Recrypt(options);
  }
...
util.inherits(Recrypt, Service);

Recrypt.dependencies = [];

Recrypt.DEFAULT_MAX_TXIDS = 1000;
Recrypt.DEFAULT_MAX_HISTORY = 50;
Recrypt.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Recrypt.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Recrypt.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Recrypt.DEFAULT_SPAWN_RESTART_TIME = 5000;
Recrypt.DEFAULT_SPAWN_STOP_TIME = 10000;
Recrypt.DEFAULT_TRY_ALL_INTERVAL = 1000;
Recrypt.DEFAULT_REINDEX_INTERVAL = 10000;
Recrypt.DEFAULT_START_RETRY_INTERVAL = 5000;
Recrypt.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Recrypt.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Recrypt.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  rpcuser: 'recrypt',
  rpcpassword: 'recryptpassword',
  uacomment: 'bitcore'
};

Recrypt.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Recrypt.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Recrypt.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Recrypt.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Recrypt.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Recrypt.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Recrypt.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Recrypt.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Recrypt.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Recrypt.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Recrypt.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Recrypt.prototype._initCaches = function() {
...
Recrypt.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Recrypt.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Recrypt.prototype.getPublishEvents = function() {
  return [
    {
...
Recrypt.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'recryptd/' + name, 'total:', this.subscriptions[name].length);
};

Recrypt.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'recryptd/' + name, 'total:', this.subscriptions[name].length);
};


Recrypt.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
Recrypt.prototype.unsubscribeBalance = function(emitter, addresses) {
...
Recrypt.prototype.unsubscribeBalanceAll = function(emitter) {
...
Recrypt.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'recryptd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Recrypt.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'recryptd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Recrypt.prototype.unsubscribeAddressAll = function(emitter) {
...
Recrypt.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Recrypt.DEFAULT_CONFIG_SETTINGS;
...
Recrypt.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Recrypt.prototype._expandRelativeDatadir = function() {
...
Recrypt.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in recryptd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in recryptd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in recryptd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './recrypt.conf');

  log.info('Using recrypt config file:', configPath);
...
Recrypt.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of recryptcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of recryptcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of recryptcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to recryptd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from recryptd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from recryptd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that recryptd is undergoing a reindex. ' +
             ...
             'of recryptcore-node services will start.');
    node._reindex = true;
  }
};

Recrypt.prototype._resetCaches = function() {
...
Recrypt.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Recrypt.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Recrypt.prototype._initChain = function(callback) {
  var self = this;
...
Recrypt.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8332
  };
...
Recrypt.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/recrypt.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/recrypt.conf';
    }
  }
  return networkPath;
};

Recrypt.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Recrypt.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('recryptd/hashblock', message.toString('hex'));
    }
  }

};

Recrypt.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Recrypt.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Recrypt Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Recrypt.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Recrypt.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Recrypt.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
Recrypt.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('recryptd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Recrypt.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('recryptd/rawtransaction', message.toString('hex'));
    }
...
Recrypt.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Recrypt.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Recrypt.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Recrypt.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Recrypt.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Recrypt Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Recrypt.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Recrypt.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Recrypt.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/recryptd.pid';
...
Recrypt.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting recrypt process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Recrypt process unexpectedly exited with code:', code);
        log.warn('Restarting recrypt child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Recrypt process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn recryptd.'));
      }
...
Recrypt.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to recryptd.'));
    }
 ...
 */
Recrypt.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Recrypt configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Recrypt.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Recrypt.prototype.syncPercentage = function(callback) {
  var self = this;
...
Recrypt.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, recryptcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Recrypt.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Recrypt.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Recrypt.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Recrypt.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in recryptcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Recrypt.prototype._getConfirmationsDetail = function(transaction) {
...
Recrypt.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Recrypt.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Recrypt.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A recryptcoin transaction id
 * @param {Function} callback
 */
Recrypt.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Recrypt.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Recrypt.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Recrypt.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Recrypt.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Recryptcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Recrypt.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getInfo = function(callback) {
...
Recrypt.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('recryptd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('recryptd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
Recrypt.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Recrypt.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Recrypt.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Recrypt.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Recrypt.prototype.getStakingInfo = function(callback) {
    var self = this,
